...
typedef struct Channel {
    ...
    int interestMask;		/* Mask of all events this channel has
                                 * handlers for. */
    struct Channel *prevChanPtr;/* Prev in list of channels currently open. */
...
static int		GetInput _ANSI_ARGS_((Channel *chanPtr, int nwanted));
    ...
    ClientData clientData;		/* NULL - unused. */
{
    ...
    Channel *nextChanPtr, *prevChanPtr;	/* Iterates over open channels. */


    for (chanPtr = firstChanPtr; chanPtr != (Channel *) NULL;
	     chanPtr = nextChanPtr) {
	prevChanPtr = chanPtr->prevChanPtr;
	...
	if (prevChanPtr) {
	    nextChanPtr = prevChanPtr->nextChanPtr;
	} else {
	    nextChanPtr = firstChanPtr;
	}
	if (nextChanPtr == chanPtr) {
                                 ...
                                 * if the channel is readable, writable. */
{
    ...
    chanPtr->prevChanPtr = NULL;
    chanPtr->nextChanPtr = firstChanPtr;
    if (firstChanPtr) {
	firstChanPtr->prevChanPtr = chanPtr;
    }
    ...
    int errorCode;			/* Status of operation so far. */
{
	...
	if (chanPtr->nextChanPtr) {
	    chanPtr->nextChanPtr->prevChanPtr = prevChanPtr;
	}
    }
    ...
    int toRead;			/* Maximum number of characters to read. */
{
    ...
    for (copied = 0; copied < toRead; copied += copiedNow) {
        ...
        if (copiedNow == 0) {
            ...
            result = GetInput(chanPtr, toRead - copied);
            if (result != 0) {
                if (result == EAGAIN) {
                    return copied;
                }
