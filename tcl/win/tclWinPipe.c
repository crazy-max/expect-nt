...
#define PIPE_MESSAGE	WM_USER+20
...
typedef struct PipeInfo {
    ...
    TclWinReaderInfo *readerInfo; /* File reader state */
    int refCount;		/* Reference count for free routine */

    struct PipeInfo *nextLPtr;	/* Pointer to next created pipe */ 
...
static PipeInfo *firstPipePtr = NULL;

/*
 * Every open pipe has an entry on the following list.
 */

static PipeInfo *pipeList = NULL;
...
/*
 * Pipe notification window.  This window is used to receive pipe
 * notification events.
 */

static HWND pipeWindow = NULL;

/*
 * Window class for creating the pipe notification window.
 */

static ATOM pipeClass;
...
static void	PipeFreeProc(ClientData clientData);
...
static int	PipeOutputProc(ClientData instanceData, char *buf, int toWrite,
		    int *errorCode);
static LRESULT CALLBACK PipeProc (HWND hwnd, UINT message, WPARAM wParam,
		    LPARAM lParam);
...
PipeInit()
{
    ...
    pipeClass = RegisterClass(&class);
    if (!pipeClass) {
	return;
    }
    pipeWindow = CreateWindowEx(0, (LPCTSTR)pipeClass, "TclPipe",
	    WS_OVERLAPPED, 0, 0, 0, 0, NULL, NULL,
	    TclWinGetTclInstance(), NULL);
    if (pipeWindow == NULL) {
	TclWinConvertError(GetLastError());
	UnregisterClass((LPCTSTR)pipeClass, TclWinGetTclInstance());
	return;
    }
 ...
 *	to register interest in a pipe event, and the event has
 *	occurred.
 *
 * Results:
 *	0 on success.
 *
 * Side effects:
 *	The flags for the given pipe are updated to reflect the
 *	event that occured.
 *
 *----------------------------------------------------------------------
 */
    ...
    LPARAM lParam;
{
    ...
    if ((hwnd != pipeWindow) || (message != PIPE_MESSAGE)) {
    ...
    /*
     * Find the specified pipe on the pipe list and update its
     * check flags.
     */

    for (pipePtr = pipeList; pipePtr != NULL; pipePtr = pipePtr->nextLPtr) {
	if (pipePtr->readerInfo->rHandle == pipe) {
	    pipePtr->readerInfo->readyMask |= event;
	    break;
	}
    }
    return 0;
    ...
    int flags;			/* Event flags as passed to Tcl_DoOneEvent. */
{
    ...
    /*
     * Check to see if there is a watched pipe.  If so, poll on Win32s.
     */
				 ...
				 * process. */
{
    ...
    if (image[0] != '\0') {
	Tcl_DStringAppend(&cmdLine, image, strlen(image));	
	Tcl_DStringAppend(&cmdLine, " ", 1);	
    }
    BuildCommandLine(argc, argv, &cmdLine);
...
static int
ApplicationType(interp, originalName, fullPath, imagePath)
    ...
    char *imagePath;
{
	...
	if (header.e_magic == 0x2123 ) { /* #! */
	    ...
	    ReadFile(hFile, (void *) tmpBuffer, MAX_PATH, &read, NULL);
	    /*
	    ...
	    *dpnt = '\0';
	    ...
	    Tcl_TranslateFileName(interp, scriptName, &execBuffer);
    ...
    if (applType == APPL_NONE) {
	/*
	 * If we have a NULL imagePath, it means we are searching for the command
	 * interpreter.  We don't record the errors here, since we may
	 * have to look at several possibilities.
	 */
	if (imagePath != NULL) {
	    TclWinConvertError(GetLastError());
	    Tcl_AppendResult(interp, "couldn't execute \"", originalName,
    ...
    Tcl_Pid *pidPtr;		/* An array of process identifiers. */
{
    ...
    if (readFile) {
	infoPtr->readerInfo = TclWinReaderNew(pipeWindow, PIPE_MESSAGE,
	    ((WinFile*)infoPtr->readFile)->handle);
    } else {
	infoPtr->readerInfo = NULL;
    }
    ...
    Tcl_Interp *interp;		/* For error reporting. */
{
    ...
    /*
     * Remove the pipe from the list of all pipes.
     */

    for (nextPtrPtr = &pipeList, infoPtr = *nextPtrPtr; infoPtr != NULL;
	    nextPtrPtr = &infoPtr->nextLPtr, infoPtr = *nextPtrPtr) {
	if (infoPtr == (PipeInfo *)pipePtr) {
	    *nextPtrPtr = infoPtr->nextLPtr;
	    break;
	}
    }
 ...
 * PipeSetOptionProc --
 *
 *	Sets an option on a channel.  Used internally to set 
 *	
 *
 * Results:
...
static int		
PipeSetOptionProc(instanceData, interp, optionName, value)
    ...
    char *value;		/* New value for option. */
{
    ...
    pipePtr = (PipeInfo *) instanceData;
    handle = (HANDLE) ((WinFile*)pipePtr->readFile)->handle;

    if (handle == NULL) {
	Tcl_AppendResult(interp, "not opened for reading", NULL);
	return TCL_ERROR;
    }

    len = strlen(optionName);
    if ((len > 1) && (strncmp(optionName, "-iomode", len) == 0)) {
	len = strlen(value);
	if (len > 1) {
	    if (strncmp(value, "overlapped", len) == 0) {
		pipePtr->readerInfo->overlappedIO = 1;
	    } else if (strncmp(value, "synchronous", len) == 0) {
		pipePtr->readerInfo->overlappedIO = 0;
	    } else {
                if (interp != NULL) {
                    Tcl_AppendResult(interp,
			"bad value for -iomode: should be one of ",
			"overlapped or synchronous", NULL);
		    return TCL_ERROR;
		}
	    }
	}
