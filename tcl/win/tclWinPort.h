...
/*
 * Used by the rType field below
 */

#define FILE_TYPE_COM		0x1006
#define FILE_TYPE_CONSOLE	0x1007

/*
 * This structure describes per-instance data for a file reader thread
 */

typedef struct TclWinReaderInfo {
    HANDLE postWin;		/* Window to post to when data is available */
    DWORD postMsg;		/* Message to post when data is available */
    int readyMask;		/* Events that need to be checked on */
    int eof;			/* Are we at the end of file? */ 
    int async;			/* Asynchronous or synchronous? */
    int overlappedIO;		/* Use overlapped or non-overlapped IO */
    OVERLAPPED rOverResult;	/* Overlapped read result */
    HANDLE rSemaphore;		/* Semaphore for read data availability */
    HANDLE rEvent;		/* Event for read data availability */
    HANDLE rGoEvent;		/* Event used to allow console input to
				 * go forward.  By holding off, an extension
				 * like expect can set the console mode.
				 * This is useful for disabling echo */
    HANDLE rHandle;		/* Handle for reading pipe */
    DWORD rType;		/* Win32 type for the handle */ 
    int rHandleClosed;		/* Set if the handle was closed */
    HANDLE rThread;		/* Reader thread handle if running */
    char *rData;		/* Buffer with currently available data */
    int rDataLen;		/* Length of currently available data */
    int rDataPos;		/* Main thread reader position */
    int refCount;
} TclWinReaderInfo;
...
EXTERN DWORD WINAPI	TclWinReaderThread (LPVOID *arg);
