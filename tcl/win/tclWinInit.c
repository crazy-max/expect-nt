    ...
    Tcl_Interp *interp;
{
	...
	if ((RegOpenKeyEx(HKEY_LOCAL_MACHINE,
		"Software\\Mitel\\Tcl\\" TCL_VERSION, 0, KEY_READ, &key)
		== ERROR_SUCCESS)
    ...
    Tcl_DStringFree(&ds);
    TclWinThreadId = GetCurrentThreadId();
}

/*
 *----------------------------------------------------------------------
 ...
 *----------------------------------------------------------------------
 *
 * TclPlatformAsyncMark --
 *
 *	Process asynchronous event on Windows
 *
 * Results:
 *	None
 *
...
void
TclPlatformAsyncMark(async)
    Tcl_AsyncHandler async;		/* Token for handler. */
{
    /*
     * Need a way to kick
     * the Windows event loop and tell it to go look at asynchronous
     * events.
     */
    PostThreadMessage(TclWinThreadId, WM_USER, 0, 0);
}
