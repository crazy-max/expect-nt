 ...
 */
void
DumpExternals(PIMAGE_SYMBOL pSymbolTable, FILE *fout, unsigned cSymbols, int type)
{
    unsigned i;
    PSTR stringTable;
    char *s, *f;
    ...
    for ( i=0; i < cSymbols; i++ ) {
	if (pSymbolTable->SectionNumber > 0 && pSymbolTable->Type == type) {
...
DumpObjFile(PIMAGE_FILE_HEADER pImageFileHeader, FILE *fout, int full)
{
	...
	/* Functions */
	DumpExternals(PCOFFSymbolTable, fout, COFFSymbolCount, 0x20);

#if 0
	/* Global Variables */
	DumpExternals(PCOFFSymbolTable, fout, COFFSymbolCount, 0x00);
#endif
    }
}
