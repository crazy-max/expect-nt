 ...
 *
 * SCCS: @(#) tkWinButton.c 1.11 97/06/18 15:45:07
 */
...
InitBoxes()
{
    HMODULE module = TkWinGetTkModule();
    HRSRC hrsrc;
    HGLOBAL hblk;
...
UpdateButtonDefaults()
{
    ...
    for (specPtr = BltpButtonConfigSpecs; specPtr->type != TK_CONFIG_END;
	    specPtr++) {
	if (specPtr->offset == Tk_Offset(BltButton, borderWidth)) {
...
BltpComputeButtonGeometry(butPtr)
    register BltButton *butPtr;	/* Button whose geometry may have changed. */
{
    ...
    LPARAM lParam;
{
	...
	case BN_CLICKED: {
		...
		code = TkInvokeButton((BltButton*)butPtr);
