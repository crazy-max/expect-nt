...
#ifdef TCL_MEM_DEBUG
#  define ckfree  Tcl_DbCkfree
#else
#  if USE_TCLALLOC
#     define ckfreeproc Tcl_Free
#  else
#     define ckfreeproc free
#  endif
#endif /* TCL_MEM_DEBUG */
...
#ifdef __WIN32__
#define MAX_READS	1	/* Trying to use Tcl_Read() when no data
				 * is available leads to an EOF condition.
				 * Avoid by only doing one Tcl_Read per
				 * notify */
#else
...
static void	BackgroundProc _ANSI_ARGS_((ClientData clientData,
		    int mask));
static int	BgExecCmd _ANSI_ARGS_((ClientData clientData,
		    Tcl_Interp *interp, int argc, char **argv));
static char *	CleanupProc _ANSI_ARGS_((ClientData clientData,
...
static void	StatusProc _ANSI_ARGS_((ClientData clientData));
...
typedef struct {
    ...
    Tcl_Channel outputChan;	/* Channel for output pipe.  */
    Tcl_Channel errorChan;	/* Channel for error file. */

    Buffer buffer;		/* Buffer storing subprocess' stdin/stderr */

    int numPids;		/* Number of processes created in pipeline */
    Tcl_Pid *pidArr;		/* Array of process Ids. */
    ...
    Tcl_FreeProc **freeProcPtr;	/* Memory deallocation scheme to use */
{
	...
	*freeProcPtr = (Tcl_FreeProc *)ckfreeproc;
 ...
 */
static int
AppendOutputToBuffer(chan, bufferPtr)
    Tcl_Channel chan;
    Buffer *bufferPtr;
{
	...
	numBytes = Tcl_Read(chan, array, bytesLeft);

	{
	    char buf[1024];
	    sprintf(buf, "Tcl_Read called: numBytes=%d\n", numBytes);
    ...
    int flags;
{
	...
	Tcl_DeleteChannelHandler(bgPtr->outputChan, BackgroundProc,
				 (ClientData)bgPtr);
	Tcl_Close(bgPtr->interp, bgPtr->outputChan);
    }
	...
	*bgPtr->donePtr = TRUE;
	...
	if (bgPtr->errVar != NULL) {
	     ...
	     */
	    ClientData handle;
	    Tcl_GetChannelHandle(bgPtr->errorChan, TCL_READABLE, 
				 (ClientData *) &handle);

#ifdef __WIN32__
	    if (SetFilePointer(handle, 0, NULL, FILE_BEGIN) == 0xffffffff)
#else
	    if (lseek(handle, 0L, 0) < 0)
#endif
	    {
		...
		ResetBuffer(&(bgPtr->buffer));
		do {
		    status = AppendOutputToBuffer(bgPtr->errorChan,
			&(bgPtr->buffer));
	...
	Tcl_Close(bgPtr->interp, bgPtr->errorChan);
    }
    ...
    ClientData clientData;
{
    ...
    *((int *)&lastStatus) = 0;
    numLeft = 0;
    for (i = 0; i < bgPtr->numPids; i++) {
	code = Tcl_WaitPid(bgPtr->pidArr[i], &waitStatus, WNOHANG);
	if (code == 0) {	/*  Process has not terminated yet */
	    if (numLeft < i) {
		bgPtr->pidArr[numLeft] = bgPtr->pidArr[i];
	    }
	    numLeft++;
	} else if (code == bgPtr->pidArr[i]) {
	    /*
	...
	bgPtr->timerToken = Tcl_CreateTimerHandler(1000, StatusProc,
	    (ClientData)bgPtr);
    } else {
    ...
    int mask;			/* Not used. */
{
    ...
    status = AppendOutputToBuffer(bgPtr->outputChan, &(bgPtr->buffer));
    ...
    bgPtr->timerToken = Tcl_CreateTimerHandler(0, StatusProc, (ClientData)bgPtr);
    ...
    char **argv;		/* Argument strings. */
{
    ...
    bgPtr->numPids = -1;
    bgPtr->outputChan = bgPtr->errorChan = NULL;
    ...
    errPipePtr = NULL;
    if (bgPtr->errVar != NULL) {
	errPipePtr = &errPipe;	/* Error variable designated */
    }
    numPids = TclCreatePipeline(interp, argc - i, argv + i, &(bgPtr->pidArr),
	NULL, &outPipe, errPipePtr);
    ...
    if (outPipe) {
	bgPtr->outputChan = TclpCreateCommandChannel(outPipe, outPipe, NULL,
						     0, NULL);
	if (bgPtr->outputChan == NULL) {
	    goto errorOnSetup;
	}
    }
    if (errPipe) {
	bgPtr->errorChan = TclpCreateCommandChannel(errPipe, errPipe, NULL,
						     0, NULL);
	if (bgPtr->errorChan == NULL) {
	    goto errorOnSetup;
	}
    }
	...
	Tcl_SetChannelOption(interp, bgPtr->outputChan, "-blocking", "0");

	Tcl_CreateChannelHandler(bgPtr->outputChan, TK_READABLE,
	    BackgroundProc, (ClientData)bgPtr);
    }
	...
	for (i = 0; i < numPids; i++) {
	    Blt_AppendInt(interp, (int) bgPtr->pidArr[i]);
	}
    } else {
	int exitCode;
	int done;
