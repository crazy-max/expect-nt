 ...
 *
 * SCCS: @(#) tkWinEmbed.c 1.19 97/08/11 09:49:01;
 */
...
static void		EmbedWindowDeleted _ANSI_ARGS_((HWND hwnd));

/*
 *----------------------------------------------------------------------
 *
    ...
    XEvent *eventPtr;			/* ResizeRequest event. */
{
	...
	/* The container is gone, delete the embedded window as well */
	EmbedWindowDeleted(containerPtr->parentHWnd);
    }
}

/*
...
static void
EmbedWindowDeleted(hwnd)
    HWND hwnd;			/* Handle to the window that was deleted. */
{
    ...
    while (1) {
	if (containerPtr->embeddedHWnd == hwnd) {
	    containerPtr->embeddedHWnd = NULL;
	    containerPtr->embeddedPtr = NULL;
	    break;
	}
