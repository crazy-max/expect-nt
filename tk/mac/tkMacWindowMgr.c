 ...
 *
 * SCCS: @(#) tkMacWindowMgr.c 1.50 97/08/07 14:47:39
 */
...
Tk_Window tkMacFocusWin = NULL; 	 /* Window to which all mouse
...
static int 	GenerateActivateEvents _ANSI_ARGS_((EventRecord *eventPtr,
			Window window));
static int 	GenerateFocusEvents _ANSI_ARGS_((EventRecord *eventPtr,
			Window window));
 ...
 *
 * Side effects:
 *	May change the window used by Key events.
 *
 *----------------------------------------------------------------------
 */
    ...
    Time time)
{
    ...
    display->request++;
    tkwin = Tk_IdToWindow(tkDisplayList->display, focus);
    if (tkwin == NULL) {
	return;
    }
 ...
 * TkpChangeFocus --
 *
 *	This procedure is a syub on the Mac because we always own the
				 ...
				 * topLevelPtr's application. */
{
    ...
    /*
     * Change the focus on the Mac.  (The same as XSetFocus.)  Don't
     * set the focus to a window that's marked override-redirect.
     */

    if (winPtr->atts.override_redirect) {
	return;
    }
    tkDisplayList->display->request++;
    tkwin = Tk_IdToWindow(tkDisplayList->display, winPtr->window);
    if (tkwin == NULL) {
	return;
    }
    ...
    Window window)		/* Root X window for event. */
{
    ...
    /*
     * The focus window is set during FocusIn events and
     * explicit calls to XSetInputFocus.  We just use that
     * value.  Tk will make sure the event gets to the *real*
     * window the focus belongs to.
     */
    tkwin = tkMacFocusWin;
    if (tkwin == NULL) {
	return false;
...
GeneratePollingEvents()
{
     ...
     */
    Tk_UpdatePointer(tkwin, whereGlobal.h,  whereGlobal.v, TkMacButtonKeyState());
    ...
    EventRecord *eventPtr)
{
	...
	case activateEvt:
	    ...
	    eventFound |= GenerateFocusEvents(eventPtr, window);
	    break;
	case getFocusEvent:
