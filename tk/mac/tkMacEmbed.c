 ...
 *
 * SCCS: @(#) tkMacEmbed.c 1.5 97/08/11 09:53:19
 */
    ...
    XEvent *eventPtr;			/* ResizeRequest event. */
{
	...
	if (containerPtr->embedded != None) {
	    errHandler = Tk_CreateErrorHandler(eventPtr->xfocus.display, -1,
            ...
            /*
             * The Mac implementation of XSetInputFocus does not generate 
             * all the needed events.  So I have copied GenerateFocusEvents, 
             * since that seems to do the right job.
             */

            TkpChangeFocus(TkpGetWrapperWindow(containerPtr->embeddedPtr), 1);
            GenerateFocusEvents(containerPtr->parentPtr->mainPtr->focusWinPtr,
		    containerPtr->embeddedPtr);

            containerPtr->embeddedPtr->mainPtr->focusWinPtr =
		containerPtr->embeddedPtr;
 ...
 *
 *      This is copied from tkFocus.c.  I need it to mock up the
 *      behavior of XSetInputFocus more accurately that the MacTk version does,
 *      but since it is not exported yet, I have to copy it.  The other 
 *      difference is that I don't pass the special GENERATED_EVENT_MAGIC flag,
 *      since this is supposed to come straight from X...  
 *
 * Results:
 *	None.
 *
...

{
    ...
    /* event.xfocus.send_event = ((Bool) 0x547321ac);*/
